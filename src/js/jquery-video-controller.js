...
(function ($) {
	...
	$.videoController = function (element, options) {
		...
		var initLayout = function ()
		{
		    ...
		    handle = document.getElementById("vgalHandle");
		    handleVal = document.getElementById("vgalHandleVal");
		    durationVal = document.getElementById("vgalTotalVal");
		    vgalRingCanvas = document.getElementById("vgalRingCanvas");
		    vController = document.getElementById("vgalController");
		    dragger = document.getElementById("vgalDragger");
		    ...
		    $("#vgalNext").mousedown(function (event) {
		        ...
		        if (nIndex > plugin.settings.videos.length-1) { nIndex = 0; }
		        window.location.hash = "Videos/" + nIndex;
		    });
		    $("#vgalPre").mousedown(function (event) {
		        var nIndex = currentIndex - 1;
		        if (nIndex < 0) { nIndex = plugin.settings.videos.length - 1; }
		        window.location.hash = "Videos/" + nIndex;
		    });
            */
		        ...
		        }else
		        {
		            ...
		            if (!isDraggingHandle) {
		                ...
		                if (md.dX < 200 && md.dY < 200) {
		                    $(".vgalEx").css("opacity", "1");//show extra controls
		                }
		                else {
		                    $(".vgalEx").css("opacity", "0");
		                }
		            }
		...
		var addDraggerLogic = function()
		{
		    ...
		    $(dragger).draggable({
		        ...
		        drag: function (e, ui) {
		            ...
		            var ref = 0;
		            if (ang > 0) {
                        //first 50% are from 3(oclock) to 9 (through 12)
		                ref = (180 - ang) / 360;
		            }
		            else {
		                //last 50% are from 9(oclock) to 3 (through 6)
		...
		{
		    if ($(video)[0].paused) {
		        $(vController).find(".vgalLine").addClass("play");
		    } else {
		        $(vController).find(".vgalLine").removeClass("play");
		    }
		}
		...
		var loadVideo = function () {
		    ...
		    //var mp4Vid = document.getElementById('vgalPlayerSrc');
		    //mp4Vid.src = vURL;
		    video.src = vURL;
		    //$(video).attr("src", vURL);
		...
		var updateProgress_V1 = function (prog) {

		    _deg = 360 * prog / 100;// _deg + 1; if (_deg > 360) { _deg = 0; }
		    var centerP = 480 / 2;
		...
		plugin.playVideoByURL = function (url) { playVideoByURL(url); }
		var playVideoByURL = function (url) {
		    _currentVideoURL = url;
		    loadVideo();
		}

		plugin.playVideoByIndex = function (index) { playVideoByIndex(index); }
		var playVideoByIndex = function (index) {
		        currentIndex = index;
		        loadVideo();
		}
		...
		var setControlsActive = function (isActive) {
		    _isControlsActive = isActive;
		    //alert("_isControlsActive=" + _isControlsActive);
		}

	    //updateUI
		plugin.updateUI = function () { updatePlayPauseMode(); }


		plugin.stopVideo = function (index) { stopVideo(); }
	...
	$.fn.videoController = function (options) {
		return this.each(function () {
			if (undefined == $(this).data('videoController')) {
				var plugin = new $.videoController(this, options);
				$(this).data('videoController', plugin);
			}
		});
	}
